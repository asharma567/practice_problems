...
def find_all_palies(L, R):
    if L == R: return is_palindrome(L)
        
    range_of_ints = range(L, R + 1)
    
    lis_of_pali = [find_n_grams(range_of_ints, i) for i in range(len(range_of_ints))]
    
    list_of_even_palies = [find_num_of_palindromes(tup) for sub_list in lis_of_pali for tup in sub_list if find_num_of_palindromes(tup) % 2 == 0]
    
    #filter out 0s
    count_of_even_palies = len([x for x in list_of_even_palies if x != 0])
    
    if count_of_even_palies in (0,1):
        x = len([find_num_of_palindromes(tup) for sub_list in lis_of_pali for tup in sub_list if find_num_of_palindromes(tup) != 0])        
        return 1 if x > 0 else 0
    return count_of_even_palies
