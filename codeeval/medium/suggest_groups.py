...
def get_user_recommendations(user_domain_dict, user_friends_dict):
    ...
    for user in user_friends_dict:
        ...
        user_recommendations[user] = [domains for domains, weight in weighted_domain_recs.iteritems() \
                                        if weight >= .5 and domains not in user_domain_dict[user]] 
...
def main(filename):
    ...
    for user in sorted(users):
        if output_recs[user]:
            out_str  = user + ':' + ','.join(sorted(output_recs[user]))
            print out_str.strip()
